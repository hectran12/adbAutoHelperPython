...
class textDetection:
    ...
    """
        getTextInImage: get text in image using EasyOCR
        file_path: file path
        return: dict (text, vertices)
    """
...
class autoDeviceADBHelper:
    ...
    """
        ...
        @return: bool
    """
    def installApk (self, path: str, timeout=60) -> bool:
        try:
    ...
    def scrollUntilFindText (self,
        ...
        try:
            for round in range(roundOut):
                if self.findText(text=text, refind=1, timeout=1, checkInLine=checkInLine):
                    return True
                else:
    ...
    """
        isInstall: check install
        @param packageName: package name
        @return: bool
    """
    def isInstall (self, packageName: str) -> bool:
        packages = self.getPackageInstalled()
    ...
    """
        deleteAllAccount: delete all account
        @return: bool

    """

    def openListAccount (self) -> bool:
        try:
            self.objAdb.execute(['adb', '-s', self.deviceId, 'shell', 'am', 'start', '-a', 'android.settings.SYNC_SETTINGS'])
            return True
        except Exception as e:
            raise handleException(f'An error occurred: {e}')
    ...
    """
        fixedServerADB: fixed server adb
        @return: bool
    """
    def fixedServerADB (self) -> bool:
        try:
    ...
    """
        onAlwaysActiveScreen: on always active screen
        @return: bool
    """
    def onAlwaysActiveScreen (self) -> bool:
        try:
            # adb shell svc power stayon true
            result, err = self.objAdb.execute(['adb', '-s', self.deviceId, 'shell', 'svc', 'power', 'stayon', 'true'])
            if err:
                raise handleException(f'An error occurred: {err}')
        except Exception as e:
            raise handleException(f'An error occurred: {e}')
    ...
    """
        ...
        @return: bool
    """
    def clearNotification (self) -> bool:
        try:
            # adb shell service call notification 1
            result, err = self.objAdb.execute(['adb', '-s', self.deviceId, 'shell', 'service', 'call', 'notification', '1'])
            if err:
                raise handleException(f'An error occurred: {err}')
        except Exception as e:
            raise handleException(f'An error occurred: {e}')
